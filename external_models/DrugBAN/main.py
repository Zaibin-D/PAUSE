...
def sync_output_dir_seed(cfg, data, split):
    """Keep RESULT.OUTPUT_DIR aligned with SOLVER.SEED without changing training logic."""
    output_dir = str(cfg.RESULT.OUTPUT_DIR)
    seed_dir = f"seed_{int(cfg.SOLVER.SEED)}"
    if output_dir.strip().replace("\\", "/").rstrip("/") in {"./result", "result"}:
        cfg.RESULT.OUTPUT_DIR = f"./results/{data}/{split}/{seed_dir}"
        print(f"[DrugBAN] output dir: {cfg.RESULT.OUTPUT_DIR}", flush=True)
        return
    synced = re.sub(
        r"(^|[/\\])seed_\d+([/\\]?)$",
        lambda match: f"{match.group(1)}{seed_dir}{match.group(2)}",
        output_dir,
    )
    cfg.RESULT.OUTPUT_DIR = synced
    if synced != output_dir:
        print(f"[DrugBAN] synchronized output dir: {output_dir} -> {synced}", flush=True)
...
def main():
    ...
    sync_output_dir_seed(cfg, args.data, args.split)
    ...
    if not cfg.DA.TASK:
        ...
        val_target_dataset = DTIDataset(df_train_target.index.values, df_train_target)
    ...
    if not cfg.DA.USE:
        ...
        if not cfg.DA.TASK:
            ...
            val_generator = DataLoader(val_target_dataset, **params)
